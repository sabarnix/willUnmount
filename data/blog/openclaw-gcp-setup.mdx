...
Most cloud setups cost between twenty and fifty dollars a month. You can hit the ten dollar target by using a schedule.

### Why skip 24/7?

Running this 24 hours a day costs about ten dollars and ten cents per month. That puts you just over the free credit limit.

### The Schedule Benefit

If you turn the agent off at night (10 PM to 9 AM), the total cost drops to about six dollars and forty cents per month. This gives you a three dollar and sixty cent buffer for heavy AI usage without getting a bill.

---

## Why this setup works

Hosting an AI agent on a tight budget requires a few clever architectural choices. First, we use Spot Instances instead of regular ones. These allow you to use Google's spare capacity at a massive 60 to 91 percent discount. While Google can technically reclaim the machine if a full-paying customer needs it, a quick 30-second restart every few days is a fair trade for saving over 100 dollars a year.

The biggest challenge with Spot instances is the IP address change. Every time the machine restarts or wakes up from its nightly schedule, Google assigns it a new public address. This would normally break a traditional domain name and require a static IP reservation. However, Google charges about three dollars and thirty cents a month just to hold that address while the VM is off, which would eat over 30 percent of your total budget.

This is where Tailscale and MagicDNS come in. Tailscale creates a secure, private network between your devices, while its MagicDNS feature assigns your VM a permanent hostname (like `http://openclaw-agent`). This name never changes, even when the underlying IP does. By using this setup, you get a reliable, encrypted connection to your dashboard that is completely invisible to the public internet, all while keeping your costs at zero for networking and security.
...
2.  Fill in the Details:
    ...
    *   Project ID: Google will generate one automatically (example: `ai-agent-45123`). **Copy this ID down**—you will need it for Step 3.
...
*   `--provisioning-model=SPOT`: This is the "Magic" setting. It gives you the server at a **90 percent discount** by using Google's spare capacity.
...
## Step 4: Secure Private Access (Tailscale and MagicDNS)
...
![Your Live OpenClaw Dashboard](/static/images/openclaw_dashboard.png)
...
## Step 7: Configure and Pair OpenClaw
...
Open your config file on the server: `nano ~/.openclaw/openclaw.json`. Update the `allowedOrigins` to include your Tailscale name:
...
### 7.3 Pair your Device

1.  Open the Dashboard: Go to your Tailscale URL (example: `https://openclaw-agent...`) in your browser.
...
To stay under the ten dollar credit limit, you should turn your VM off when you aren't using it (example: while you sleep).
...
Note: Use a valid IANA timezone string (example: `Asia/Kolkata`, `America/New_York`, `UTC`). You can find a full list on Wikipedia.
...
| Item | Cost |
| :--- | :--- |
| Compute (Spot) | roughly 1.40 dollars |
| Storage (20GB) | roughly 2.00 dollars |
| AI (Gemini 2.5 Flash) | roughly 3.00 dollars |
| Total | **roughly 6.40 dollars** |
